...
@task()
def clean(ctx):
    for directory in ('build', 'dist', 'DatasheetExtractor.egg-info'):
...
def _get_pipy_json():
    import requests
    response = requests.get('https://pypi.org/pypi/DatasheetExtractor/json')
...
def get_github_tar_sha(ctx):
    ...
    result = ctx.run('git describe --tags --abbrev=0 --always', hide='out')
    tag = result.stdout.strip()
    url = 'https://github.com/FabriceSalvaire/DatasheetExtractor/archive/{}.tar.gz'.format(tag)
    print('Get', url)
    import hashlib
    import requests
